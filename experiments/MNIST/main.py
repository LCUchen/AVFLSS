...
from dataset.MNIST.load_mnist_data import load_mnist_data
from torch.utils.data import DataLoader, TensorDataset
from experiments.conf import dataset_config, max_epoch, home
...
def run_experiment(party):
    epoch = 0
    while epoch < max_epoch:
        party.train(epoch)
        epoch += 1
    party.save_result(home + 'result/mnist-%d-%d' % (party.id, party.party_num))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=int, help='0: Synchronous mode; 1: Homomorphic Encryption mode; 2: '
                                                 'Asynchronous mode; 3: dp mode', default=3)
    ...
    if opt.id == -1:
        ...
        train_data, train_label = load_mnist_data('../../dataset/MNIST', 'train')
        train_data = vertical_partition_data(train_data, opt.party_num)[opt.id]
        test_data, test_label = load_mnist_data('../../dataset/MNIST', 't10k')
        test_data = vertical_partition_data(test_data, opt.party_num)[opt.id]
        local_model = LocalModel(input_dim=len(train_data[0][0]))
        ...
        if opt.mode == 0:
            ...
            party = DP_Participant(opt.name, opt.id, opt.party_num, train_data_loader, test_data_loader,
                                   local_model, dataset_config['MNIST']['loss_function'], opt.batch_size)
        thread = threading.Thread(target=party.accept_connect)
        thread.setDaemon(True)
        thread.start()
        run_experiment(party)
