...
from dataset.parkinsons.load_parkinsons import make_data, vertical_partition_data
...
def run_experiment(party, mode):
    ...
    while epoch < max_epoch:
        party.train(epoch)
        epoch += 1
    party.save_result(home + 'result/parkinsons-%d-%d-%d-1' % (party.id, party.party_num, mode))
...
if __name__ == "__main__":
    ...
    if opt.id == -1:
        ...
        if opt.id == 0:
            test_data = {'data': vertical_partition_data(test_data, opt.party_num), 'label': torch.tensor(test_label).float()}
        else:
            test_data = None
        ...
        if opt.mode == 0:
            party = Sync_Participant(opt.name, opt.id, opt.party_num, train_data_loader, test_data,
                                     local_model, dataset_config['parkinsons']['loss_function'], opt.batch_size)
        elif opt.mode == 1:
            party = Participant(opt.name, opt.id, opt.party_num, train_data_loader, test_data,
                                local_model, dataset_config['parkinsons']['loss_function'], opt.batch_size)
        elif opt.mode == 2:
            party = Async_Participant(opt.name, opt.id, opt.party_num, train_data_loader, test_data,
                                      local_model, dataset_config['parkinsons']['loss_function'], opt.batch_size)
        else:
            party = Async_Participant(opt.name, opt.id, opt.party_num, train_data_loader, test_data, local_model,
                                      dataset_config['parkinsons']['loss_function'], opt.batch_size, opt.mode)
        thread = threading.Thread(target=party.server_side)
        thread.setDaemon(True)
        thread.start()
