...
from dataset.cifar10.load_cifa10_data import make_data, vertical_partition_data
...
def run_experiment(party, mode):
    ...
    while epoch < max_epoch:
        party.train(epoch)
        epoch += 1
    party.save_result(home + 'result/cifar10-%d-%d-%d' % (party.id, party.party_num, mode))
    ...
    if opt.id == -1:
        ...
        if opt.mode == 0:
            party = Sync_Participant(opt.name, opt.id, opt.party_num, train_data_loader, test_data_loader,
                                     local_model, dataset_config['cifar10']['loss_function'], opt.batch_size)
        elif opt.mode == 1:
